...
# 读取Python测试用例
def read_python_test_cases():
    test_cases = []
    with open('test_python_cases.py', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # 分割测试用例
    test_functions = content.split('# 测试用例')
    for i, test_function in enumerate(test_functions[1:], 1):
        lines = test_function.strip().split('\n')
        # 找到def开头的行
        def_line = None
        for line in lines:
        ...
        if def_line:
            ...
            # 找到函数定义的起始位置
            start_idx = lines.index(def_line)
...
# 批量测试
def batch_test():
    test_cases = read_python_test_cases()
    results = []
    
    print('开始批量测试Python用例...')
    print('-' * 80)
    
    for test_case in test_cases:
        print(f'测试用例 {test_case["id"]}: {test_case["name"]}')
        result = analyze_code(test_case['code'], 'python')
        results.append({
            'test_case': test_case,
            'result': result
        })
